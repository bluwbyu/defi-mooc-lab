...
describe("Liquidation 2", function () {
    // 2000 USDT
  it("test 2000 USDT", async function () {
    ...
    const USDT_contract_input = ethers.utils.parseUnits(USDT_debt.toString(), 6); // parseUnits (6 decimal)
    ...
    console.log("[2000 USDT] Profit", utils.formatEther(profit), "ETH");
  ...
  //5000 USDT
  it("test 5000 USDT", async function () {
    await network.provider.request({
        method: "hardhat_reset",
        params: [{
          forking: {
    ...
    const profit = afterLiquidationBalance.sub(beforeLiquidationBalance);
    console.log("[5000 USDT] Profit", utils.formatEther(profit), "ETH");

    expect(profit.gt(BigNumber.from(0)), "not profitable").to.be.true;
    // writeFile('profit.txt', String(utils.formatEther(profit)), function (err) {console.log("failed to write profit.txt: %s", err)});
  });
  //10000 USDT
  it("test 10000 USDT", async function () {
    await network.provider.request({
        method: "hardhat_reset",
        params: [{
          forking: {
    ...
    const profit = afterLiquidationBalance.sub(beforeLiquidationBalance);
    console.log("[10000 USDT] Profit", utils.formatEther(profit), "ETH");
